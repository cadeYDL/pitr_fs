...
def main() -> None:
    ...
    lines += [
        ...
        f"- slice pin 行数：{space.get('slice_pin_rows', '-')}",
        f"- slice pin 编码大小：{space.get('slice_pin_bytes', '-')} bytes",
        f"- slice 聚合引用行数：{space.get('slice_ref_rows', '-')}",
        f"- retained 估算：{space.get('retained_bytes', '-')} bytes",
        f"- reclaimable 估算：{space.get('reclaimable_bytes', '-')} bytes",
        f"- GC 请求预计释放：{space.get('gc_estimated_bytes', '-')} bytes",
        "",
        "## 结论与优化路径",
        "",
    ]
