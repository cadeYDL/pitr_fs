...
func (n *Node) versionedHookWindow(
	...
	autoID, err := root.manager.OpenStandaloneVersion(ctx, absPath, command)
	if err != nil {
	...
	if err := root.manager.CloseStandaloneVersion(ctx, window.autoID); err != nil {
...
func (n *Node) closeWritableWindow(
	...
	closeErr := n.root.manager.CloseStandaloneVersion(finalCtx, window.autoID)
	cancel()
	n.root.setLongPath("")
