...
网络瞬断导致的镜像构建失败会自动重试三次。发布产物应由 CI 预构建，普通用户
安装时优先拉取并校验固定镜像或二进制，避免在用户机器上重复编译 JuiceFS。
...
数据库内的 `pitr_schema_state` 是 schema revision、最低兼容逻辑 revision 和
SHA256 摘要的权威账本；schema 与账本在同一个 PostgreSQL 事务中提交，宿主摘要
仅作为诊断缓存。摘要未变化时不会重复校准，因此纯二进制升级和后续容器恢复不会
重建 slice 索引。健康检查失败时，只有旧逻辑满足新 schema 的兼容契约才会自动
回退；无法证明兼容时服务保持停止并给出人工恢复提示，避免旧二进制在不兼容 schema
上继续运行。
