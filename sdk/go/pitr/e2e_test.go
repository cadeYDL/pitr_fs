...
func TestGoSDK_RealPitrd_E2E(t *testing.T) {
	...
	v1 := latestAutoVersion(t, client, ctx, scope)

	if err := os.WriteFile(filePath, []byte("go-v2"), 0o644); err != nil {
		t.Fatal(err)
	}
	v2 := latestAutoVersion(t, client, ctx, scope)
	diff, err := client.Diff(ctx, v1, v2, scope)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := client.Revert(ctx, v1, WithPath(scope)); err != nil {
		t.Fatal(err)
	}
